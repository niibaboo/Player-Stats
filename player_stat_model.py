...
Blended last-10-match form and full-season rates, priced with a
Poisson model for shots, cards, goals and assists.
"""
...
# ----------------------------------------------------------------------
# TheStatsAPI calls - FIXED PARAMS
# ----------------------------------------------------------------------

def search_player(name: str) -> dict:
    for params in [{"search": name}, {"query": name}, {"name": name}]:
        try:
            data = cached_get("/players/search", params, ttl_hours=24 * 7)
...
def search_team(name: str) -> dict:
    # Try all known param variants - fixes your 400 Bad Request
    for endpoint in ["/teams/search", "/teams"]:
        for params in [{"search": name}, {"query": name}, {"name": name}]:
            try:
                data = cached_get(endpoint, params, ttl_hours=24 * 30)
...
def save_scan(reports: list[dict], qualifying: list[dict], ranked: dict) -> None:
    ...
    data["screener"] = {
        ...
        "top_by_market": {market: [r["player_name"] for r in reps] for market, reps in ranked.items()},
    }
    OUTPUT_JSON.write_text(json.dumps(data, indent=2))
...
def print_report(report: dict) -> None:
    print(f"\n{report['player_name']} (last {report['rolling_matches_used']} matches used for form)")
...
if __name__ == "__main__":
    ...
    # Auto-detect CI - don't ask for input on GitHub Actions
